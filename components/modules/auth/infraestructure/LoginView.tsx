...
export function LoginView() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Guardamos el token en memoria (o AsyncStorage)
      console.log("Token:", access_token);
      console.log("Usuario:", user);

      Alert.alert("Bienvenido", `Hola ${user.name || username}`);
    } catch (error: any) {
      console.error(error);
...
// 🎨 Estilos en tonos azul y blanco
const styles = StyleSheet.create({
  container: {
    flexGrow: 1,
    backgroundColor: "#f4f8ff", // fondo blanco azulado
    padding: 20,
